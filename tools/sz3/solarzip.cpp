...
void convertDatToFits(const char* fitspath, const char* decPath, struct config Config){
    ...
    string path = "../deCompData/"+name+"_"+Config.algorithem+"_"+Config.errorMode+Config.N+".fits";
...
void processCompression(char* argv[], const char* resultPath, const char* filename, int N, struct config* Config, long* naxes,int index,compResult *Result){
    ...
    char cmpPath[100]; //这个名字改成从pathList中原文件名，后缀是.dat.sz
     snprintf(cmpPath, sizeof(cmpPath), "../compData/%s_HDU%d.dat.sz", filename,index);
    char *conPath = nullptr;                      
    char decPath[100] ;
     snprintf(decPath, sizeof(cmpPath), "../binaryData/De_%s_HDU%d.dat", filename,index);
...
struct pathConfig processFile(const char* fitsPath,const char* binaryDir){
    ...
    snprintf(decPath, sizeof(decPath), "../binaryData/De_%s", filename_pure.c_str()); 
...
int main(int argc, char* argv[]){
    ...
    for(int i=0;i<count;i++){
      ...
      //记录实验数据到csv文件
    if(is_test==0){
        sprintf(csvPath, "../%s_%s.csv", Config.errorMode.c_str(),Config.N.c_str());
